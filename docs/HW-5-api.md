...
Your code to create these goes in `cloudysky/app/views.py`, with some updates to  `cloudysky/app/urls.py` to ensure that they render.   When the API creates a post or a comment, the server should return HTTP code 201, created; when the API hides a post or presents data, it should return HTTP code 200.
